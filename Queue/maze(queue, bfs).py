...
def bfs_generator_func(maze, start, goal):
...
def main():
    ...
    pygame.init()
    pygame.display.set_caption("Interactive Maze with Dynamic BFS Animation")
    ...
    while running:
            ...
            if event.type == pygame.USEREVENT:
                if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                    if event.ui_element == start_bfs_button:
                        bfs_generator = bfs_generator_func(maze, start, goal)
                        bfs_path = []
                        bfs_visited = set()
                        is_bfs_running = True
                        path_length_label.set_text("Path Length: N/A")
        ...
        if is_bfs_running and bfs_generator and time_since_last_step >= step_delay:
            try:
                state, data, parent, visited = next(bfs_generator)
                bfs_visited = visited
                if state == 'done':
                    bfs_path = data
                    is_bfs_running = False
